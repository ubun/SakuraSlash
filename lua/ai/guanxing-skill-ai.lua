function judgecheck(target,reason,card)
	...
	if  target:hasSkill("tianxiang") and card:getSuitString()=="spade" then
		local changedcard = sgs.Card_Parse(("%s[%s:%d]"):format(card:objectName(),"heart", card:getNumber()))
...
function Card2Judge(card)
	...
	elseif card:isBlack() then return "supply_shortage"
	elseif card:getSuit() == sgs.Card_Diamond then return "indulgence" end
...
function solveValue(judge,player)
	if judge == "lightning" then return 4 end
		if player:getHp() >= player:getHandcardNum() then
			if judge == "indulgence" then return 1 else return 2 end
		else
			if judge == "supply_shortage" then return 1 else return 2 end
		end
