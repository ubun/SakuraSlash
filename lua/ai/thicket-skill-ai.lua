...
duanliang_skill.getTurnUseCard=function(self)
	...
	for _,acard in ipairs(cards)  do
		if (acard:getSuit() == sgs.Card_Club) and (acard:inherits("BasicCard") or acard:inherits("EquipCard")) and (self:getUseValue(acard)<(sgs.ai_use_value["SupplyShortage"] or 0)) then
			card = acard
			break
		end
	end
