...
public:
    ...
    virtual bool trigger(TriggerEvent event, ServerPlayer *kudou, QVariant &data) const{
        ...
        if(event == PhaseChange && kudou->getPhase() == Player::Finish){
            ...
            CardEffectStruct effect = data.value<CardEffectStruct>();
            if(effect.from!=kudou && effect.card->inherits("TrickCard")
                && !effect.card->inherits("DelayedTrick")
...
public:
    Rexue():TriggerSkill("rexue"){
        events << Damaged << AskForPeaches;
    }

    virtual bool triggerable(const ServerPlayer *target) const{
        return target->hasSkill(objectName()) || !target->getPile("rexue").isEmpty();
    }

    virtual bool trigger(TriggerEvent event, ServerPlayer *player, QVariant &data) const{
        Room *room = player->getRoom();
        ServerPlayer *hatto = room->findPlayerBySkillName(objectName());
        if(event == AskForPeaches){
            DyingStruct dying_data = data.value<DyingStruct>();
            repeat:
            if(!dying_data.who->getPile("rexue").isEmpty() && dying_data.who->askForSkillInvoke(objectName(), data)){
                int peach = dying_data.who->getPile("rexue").first();
        ...
        else if(event == Damaged && player == hatto){
            DamageStruct damage = data.value<DamageStruct>();
            /*QList<ServerPlayer *> Hurts = room->getOtherPlayers(hatto);
            foreach(ServerPlayer *p, Hurts)
                if(!p->isWounded())
                    Hurts.removeOne(p);
            if (Hurts.length()<1 || !hatto->askForSkillInvoke(objectName()))
                return false;
                */
...
public:
    ...
    virtual bool trigger(TriggerEvent , ServerPlayer *player, QVariant &) const{
        ...
        if(ai->isWounded() && ai->faceUp() && ai->askForSkillInvoke(objectName())){
            ai->turnOver();
...
public:
    ...
    virtual bool trigger(TriggerEvent event, ServerPlayer *ayumi, QVariant &data) const{
            DamageStruct damage = data.value<DamageStruct>();
...
public:
    ...
    virtual bool isEnabledAtPlay(const Player *yoshida) const{
        ...
        foreach(const Player *player, players){
            if(!player->isAlive() || yoshida->getHp() > player->getHp())
               return false;
        }
        return true;
    }
...
class Du0ren: public TriggerSkill{
public:
    Du0ren():TriggerSkill("#duoren"){
        events << CardResponsed;
    }
...
class Duoren:public OneCardViewAsSkill{
public:
    Duoren():OneCardViewAsSkill("duoren"){
    }
...
public:
    ...
    virtual bool onPhaseChange(ServerPlayer *mouri) const{
        ...
        if(mouri->getPhase() == Player::Start){
            QList<ServerPlayer *> Hurts = room->getAlivePlayers();
            foreach(ServerPlayer *player, Hurts)
                if(!player->isWounded())
                    Hurts.removeOne(player);
            if(Hurts.length() > 0 && mouri->askForSkillInvoke(objectName())){
...
public:
    ...
    virtual bool trigger(TriggerEvent , ServerPlayer *kazuha, QVariant &data) const{
        ...
        if(kazuha->getHandcardNum() > effect.from->getCardCount(true))
            return false;
        if(!effect.from->isAllNude() && room->askForSkillInvoke(kazuha, objectName(), data)){
            int card_id = room->askForCardChosen(kazuha, effect.from, "hej", objectName());
            if(room->getCardPlace(card_id) == Player::Hand)
...
public:
    ...
    virtual bool trigger(TriggerEvent , ServerPlayer *kyo, QVariant &data) const{
        DamageStruct damage = data.value<DamageStruct>();
        Room *room = kyo->getRoom();
        if(damage.card && damage.card->inherits("Slash")){
            QList<ServerPlayer *>players = room->getAlivePlayers();
            foreach(ServerPlayer *player, players){
                if(damage.card->getSkillName() != "shenyong_slash")
                    player->setMark("shenyong", 8);
                if(!damage.to->inMyAttackRange(player) || damage.to == player
                   || (damage.card->getSkillName() == "shenyong_slash" && player->getMark("shenyong") == 4))
                    players.removeOne(player);
            }
            if(players.length() < 1 || !kyo->askForSkillInvoke(objectName(),data))
                return false;
...
public:
    ...
    virtual bool trigger(TriggerEvent , ServerPlayer *player, QVariant &data) const{
        ...
        if(player->getPhase() == Player::Discard){
            QVariantList quzheng = megure->tag["Quzheng"].toList();
...
class QuzhengGet: public PhaseChangeSkill{
public:
    QuzhengGet():PhaseChangeSkill("#quzheng-get"){
    ...
    virtual bool onPhaseChange(ServerPlayer *player) const{
        ...
        QVariantList quzheng_cards = megure->tag["Quzheng"].toList();
        megure->tag.remove("Quzheng");

        QList<int> cards;
        foreach(QVariant card_data, quzheng_cards){
        ...
        if(megure->askForSkillInvoke("quzheng", cards.length())){
            if(megure->isLord() && megure->hasSkill("ranglu")){
                QList<ServerPlayer *> lieges = room->getLieges("zhen", megure);
                lieges << megure;
                megure = room->askForPlayerChosen(megure, lieges, "ranglu");
            }
            room->fillAG(cards, megure);

            int n = 0;
            while(!cards.isEmpty()){
                int take = room->askForAG(megure, cards, true, objectName());
                if(take == -1)
                    break;
                cards.removeOne(take);
                n++;
                room->takeAG(megure,take);
                //room->moveCardTo(Sanguosha->getCard(take), megure, Player::Hand, false);
            }

            megure->invoke("clearAG");
            if(n > 0)
                room->askForDiscard(megure, objectName(), n);
        }
...
public:
    ...
    virtual bool trigger(TriggerEvent, ServerPlayer *player, QVariant &data) const{
        ...
        if(effect.slash && player->askForSkillInvoke(objectName())){
            ...
            //Player::history.value("Slash", 0) = 0;
...
public:
    ...
    virtual bool onPhaseChange(ServerPlayer *player) const{
        ...
        if(player->getPhase() == Player::Draw){
            if(player->getKingdom()=="hei" && player!=gin && player->askForSkillInvoke(objectName())){
                gin->addMark("@heiyi");
                return true;
            }
            else if(player==gin){
                for(gin->getMark("@heiyi");gin->getMark("@heiyi")>0;gin->loseMark("@heiyi")){
                    gin->drawCards(2);
                }
            }
        }
...
/*
class Jushen: public SlashBuffSkill{
public:
    Jushen():SlashBuffSkill("jushen"){

    }

    virtual bool buff(const SlashEffectStruct &effect) const{
        effect.from->getRoom()->slashResult(effect, true);
        return true;
    }
};
*/
...
public:
    ...
    virtual bool trigger(TriggerEvent event, ServerPlayer *player, QVariant &data) const{
        ...
        if((damage.from == agasa || damage.to == agasa) && room->askForSkillInvoke(agasa,objectName())){
...
public:
    ...
    virtual bool trigger(TriggerEvent , ServerPlayer *player, QVariant &data) const{
        ...
        if(cancer->askForSkillInvoke(objectName())){
...
/*class Baomu: public OneCardViewAsSkill{
public:
    Baomu():OneCardViewAsSkill("baomu"){
    }
    virtual bool isEnabledAtPlay(const Player *player) const{
        return false;
    }
    virtual bool isEnabledAtResponse(const Player *player, const QString &pattern) const{
        return pattern.contains("peach") && player->getMark("use_baomu") == 1;
    }
    virtual bool viewFilter(const CardItem *to_select) const{
...
class Baomupre: public TriggerSkill{
public:
    Baomupre():TriggerSkill("#baomupre"){
        events << AskForPeaches;
    }
...
virtual bool trigger(TriggerEvent event, ServerPlayer *player, QVariant &data) const{
    Room *room = player->getRoom();
    ServerPlayer *agasa = room->findPlayerBySkillName("baomu");
    agasa->setMark("use_baomu",0);
    if(player->getKingdom() == "shao")
        agasa->setMark("use_baomu",1);
    return false;
}
};
*/
...
/*
class ESuitPattern: public CardPattern{
public:
    ESuitPattern(Card::Suit suit)
        :suit(suit)
    {
    }
...
void StandardPackage::addGenerals(){
    ...
    megurejyuuzou->addSkill(new QuzhengGet);
...
//    agasahiroshi->addSkill(new Baomupre);
